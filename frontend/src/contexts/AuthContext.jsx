...
export const AuthProvider = ({ children }) => {
    ...
    const handleLogin = async (username, password) => {
        try {
            ...
            if (request.status === httpStatus.OK) {
                ...
                localStorage.setItem("name", request.data.name); // ← save full name too
                setCurrentUser({ username: request.data.username, name: request.data.name });
                router("/home");
            }
        } catch (err) {
            throw err;
    ...
    const data = {
        ...
        currentUser,           // ← expose to all pages
        addToUserHistory,
        getHistoryOfUser,
        handleRegister,
        handleLogin
